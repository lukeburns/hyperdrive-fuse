...
class PosixAdapter {
  ...
  /**
   * All open fds to `path` must see the same bytes (e.g. git writing a pack file while
   * index-pack reads it). Grows and truncates update every fd, not just the caller.
   */
  _syncBufForPath (path, buf) {
    this._inFlight.set(path, buf)
    for (const h of this._fds.values()) {
      if (h && h.path === path && h.type !== 'dir') {
        h.buf = buf
      }
    }
  }
    ...
    ;(async () => {
      ...
      if (w) {
        ...
        if (this._inFlight.has(n)) {
          ...
          for (const peer of this._fds.values()) {
            if (peer && peer.path === n && peer.buf != null && peer.type !== 'dir') {
              buf = peer.buf
              break
            }
          }
      ...
      if (this._inFlight.has(n)) {
        ...
        if (b == null) {
          b = (await this.drive.get(n, { wait: true })) || Buffer.alloc(0)
          b = Buffer.from(b)
        }
      }
      return cb(null, this._allocFd({ path: n, readOnly: true, buf: b }))
    })().catch(cb)
  }
  ...
  write (fd, b, o, l, off, cb) {
    ...
    h.buf = out
    this._syncBufForPath(h.path, out)
    return cb(null, l)
  }
  ...
  truncate (path, size, cb) {
    ...
    ;(async () => {
      if (this._inFlight.has(path)) {
        const b = resize(this._inFlight.get(path), size)
        this._syncBufForPath(path, b)
        return cb(null)
      }
  ...
  ftruncate (fd, size, cb) {
    ...
    this._syncBufForPath(h.path, h.buf)
    cb(null)
  }
